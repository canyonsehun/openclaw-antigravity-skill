...
- **State directory** (`agentDir`): auth profiles, model registry, per-agent config at `~/.openclaw/agents/<id>/agent/`
- **Session store**: chat history + routing state at `~/.openclaw/agents/<id>/sessions`
...
```json5
{
  bindings: [
    ...
    {
      agentId: "family",
      match: { channel: "whatsapp", peer: { kind: "group", id: "group-jid@g.us" } },
    },
  ],
}
```
...
Each agent can override model, tools, sandbox settings, group chat patterns, and heartbeat:
